...
const GeminiAdvisor: React.FC<Props> = ({ results, inputs }) => {
  ...
  const getAdvice = async () => {
    ...
    if (!process.env.API_KEY) {
      setError("AI service is currently unavailable. Please ensure the API key is configured.");
      return;
    }
    ...
    try {
      ...
      if (err.message?.includes("403") || err.message?.includes("API key")) {
        setError("AI authentication failed. Please check your service configuration.");
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-4">
            ...
            <p className="text-slate-400 text-sm mt-1">Personalized breakdown of your earnings and regional tax efficiency.</p>
          </div>
          ...
          {loading && (
            <div className="py-6 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-sm font-medium text-red-200 animate-pulse tracking-wide">Analyzing tax data and living costs...</p>
            </div>
          )}
          ...
          {advice && (
            <div className="bg-slate-900/80 p-6 rounded-xl text-sm leading-relaxed border border-slate-700 shadow-inner animate-fadeIn relative">
               ...
               <div className="mt-8 pt-4 border-t border-slate-700 flex justify-between items-center">
                 ...
                 <span className="text-[10px] text-slate-600 uppercase tracking-widest">Powered by Gemini 3</span>
               </div>
            </div>
          )}
        </div>
