...
const PrivacyPolicy: React.FC<PrivacyPolicyProps> = () => {
  return (
    <LegalChrome
      ...
      effective="Effective date: September 15, 2026 (replaces the version of March 1, 2026)"
      links={[
    ...
    >
      ...
      <Section n={2} id="statistics" title="Anonymous usage statistics">
        ...
        <ul className="list-disc space-y-1.5 pl-5">
          ...
          <li>the interface language, your device type (phone, tablet or computer), browser family, operating-system family and, on some phones, the handset maker — never the full browser signature, screen dimensions or a device fingerprint;</li>
          ...
          <li>optional answers you choose to give: the industry you compare against, why you are calculating, whether the result matched your expectation, where you work (on-site, remote, hybrid), your age group, tenure, union membership, employer size and vacation days;</li>
        ...
        <p>
          ...
          added to the address and this browser will record nothing from then on (<code>?notelemetry=0</code> turns it back on).
          The calculator works identically either way.
        </p>
      </Section>
      ...
      <Section n={4} id="cookies" title="Cookies, local storage and analytics">
        <p>
          <strong className="text-slate-800">One cookie, and only if you sign in.</strong> Signing in sets a cookie named{' '}
          <code>cp_session</code> that keeps you signed in for 30 days. It is essential to that feature, is
          not used for tracking or advertising, and is not set unless you sign in. We use no advertising or
          third-party tracking cookies, which is why this site has no cookie banner.
        </p>
        <p>
          ...
          “has calculated before” flag. None of it is sent to us as an identifier and clearing your
          browser’s site data removes all of it.
        </p>
        <p>
          <strong className="text-slate-800">Page-view counting.</strong> A small script we run ourselves on Cloudflare records
          the page path and the domain of the referring site — no cookies, no IP address stored. We may
          also enable Cloudflare Web Analytics, our hosting provider’s cookieless page-view counter. We do
          not use Google Analytics or any advertising network.
        </p>
        <p>
      ...
      <Section n={5} id="accounts" title="Accounts and purchases">
        <p>
          An account is optional; every calculator feature works without one. If you sign in with Google or
          Apple we receive the name, email address and profile picture that provider shares, and we store
      ...
      <Section n={7} id="sharing" title="Who receives data">
        <ul className="list-disc space-y-1.5 pl-5">
          <li><strong className="text-slate-800">Service providers</strong> that process data on our behalf: Cloudflare (hosting, database, email delivery), Stripe (payments), Google and Apple (sign-in). Each acts under its own privacy commitments and receives only what its function needs.</li>
