...
export async function GET(request: Request) {
  ...
  const rows = (await (await db()).prepare('select created_at, product, from_province, to_province, stripe_session_id from purchases where user_id = ? and refunded = 0 order by created_at desc').bind(u.id).all()).results;
