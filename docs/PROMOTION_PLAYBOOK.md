# CanPay Insights Promotion Playbook

## Positioning

CanPay Insights is a free Canadian payroll calculator for workers comparing take-home pay by province. Lead with:

- Canadian payroll math: federal tax, provincial tax, CPP, EI
- Hourly, annual salary, and timesheet modes
- Province-specific landing pages
- Free to use, with optional cloud save

## Priority Channels

1. Google Search Console
   - Request indexing for `/salary-after-tax-canada`, `/cpp-ei-calculator`, and every province page.
   - Re-submit `/sitemap.xml` after each major content batch.

2. Reddit feedback posts
   - Good-fit communities: `r/PersonalFinanceCanada`, `r/SideProject`, `r/ImmigrationCanada`, local city/province subreddits.
   - Ask for feedback instead of dropping a naked link.

3. LinkedIn
   - Post from a personal profile, not only a product page.
   - Target HR, payroll, bookkeeping, newcomers, and job-search angles.

4. Backlink outreach
   - Use `/link-to-canpay` when emailing bloggers, HR resource pages, newcomer guides, and Canadian personal finance newsletters.

5. Content expansion
   - Add salary-specific pages for common searches: `$50,000 after tax Ontario`, `$60,000 after tax BC`, `$80,000 after tax Alberta`.
   - Add comparison pages: `Ontario vs Alberta take-home pay`, `BC vs Ontario salary after tax`.

## Live Long-Tail Pages

The site now includes common amount + province pages for:

- $50,000, $60,000, $65,000, $70,000, $80,000, and $100,000
- Ontario, BC, Alberta, and Quebec

Examples:

- https://canpayinsights.ca/50000-after-tax-ontario
- https://canpayinsights.ca/65000-after-tax-ontario
- https://canpayinsights.ca/70000-after-tax-bc
- https://canpayinsights.ca/80000-after-tax-alberta
- https://canpayinsights.ca/100000-after-tax-ontario

## Copy-Paste Posts

### Reddit / SideProject

I built a free Canadian payroll calculator because I kept seeing people compare salaries without knowing the actual take-home pay.

It supports province-specific estimates for federal tax, provincial tax, CPP, EI, hourly wage, annual salary, overtime, and timesheets.

I would love feedback from Canadians, especially anyone comparing job offers or moving provinces:
https://canpayinsights.ca/

### LinkedIn

Gross salary is not what lands in your bank account.

I built CanPay Insights to help Canadians estimate take-home pay by province, including federal tax, provincial tax, CPP, EI, hourly wage, annual salary, overtime, and timesheet scenarios.

Useful for job offers, relocation planning, HR conversations, and personal budgeting:
https://canpayinsights.ca/

### Outreach Email

Subject: Free Canadian payroll calculator for your resource page

Hi,

I built CanPay Insights, a free Canadian payroll calculator that estimates take-home pay by province, including federal tax, provincial tax, CPP, EI, hourly wage, annual salary, overtime, and timesheets.

It may be useful for your readers who are comparing job offers, planning a move, or budgeting around Canadian paycheques:
https://canpayinsights.ca/

There is also a link/resource page here:
https://canpayinsights.ca/link-to-canpay

Thanks,
Travis
